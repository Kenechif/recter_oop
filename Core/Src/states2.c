...
extern float previous_totaliserVol2c,
      	  	 currentValue_tv2;
...
static int time_tone2;
static uint8_t st_tone2 = 0;
uint8_t timerFlag_tone2 = 0;
uint8_t timerFlagOld_tone2 = 0;
...
uint8_t sales_type2 CCRAM = UNPROGRAMMED_SALE;

//extern float key_value2 = 0.0;
...
uint8_t _pump_max_litres2 = 0,
		...
		firstTime_authorizedSale_overflow2 = 0;
...
sStateEventMachine2 asStateEventMachine2 [] =
{
	...
	{idle_State, idleState_Handler2, {_operator_Event,_keyup_Event,_tot_error_Event, _keypress_Event,_nozzleup_Event, _auth_command_Event, _nozzledown_Event, _resetcommand_Event, _switchoffcommand_Event, _tone_Event}},
	...
	{authorised_nozzleup_State, authorised_nozzleup_State_Handler2, {_filling_pulse_Event,_pause_Event,_timeout_Event,_nozzledown_Event,_keypress_Event,_function_key_Event, _stopcommand_Event, _auth_suspendcommand_Event, _hardwarereset_Event, _hardwareerror_Event, _tone_Event}},
	{authorisation_paused_State, authorisation_paused_State_Handler2, {_resume_Event,_timeout_Event,_nozzledown_Event,_keypress_Event, _auth_resumecommand_Event,  _stopcommand_Event, _hardwarereset_Event, _hardwareerror_Event, _switchoffcommand_Event, _tone_Event}},
	{authorisation_resumed_State, authorisation_resumed_State_Handler2, {}},
	{filling_State, filling_State_Handler2, {_filling_paused_Event,_keypress_Event,_timeout_Event,_nozzledown_Event, _stopcommand_Event, _filling_suspendcommand_Event, _hardwarereset_Event, _hardwareerror_Event, _mamo_Event, _switchoffcommand_Event, _tone_Event}},
	{filling_paused_State, filling_paused_State_Handler2, {_filling_resumed_Event, _keypress_Event, _timeout_Event,_nozzledown_Event, _filling_resumecommand_Event, _stopcommand_Event, _hardwarereset_Event, _hardwareerror_Event, _switchoffcommand_Event, _tone_Event}},
	{filling_resumed_State, filling_resumed_State_Handler2, {}},
	{keypad_entry_State, keypad_entry_State_Handler2, {}},
	{operator_State, operator_State_Handler2, {_keypress_Event, _tone_Event}},
	{savesettings_State, savesettings_State_Handler2, {_keypress_Event, _tone_Event}},
	{read_flash_State, read_flash_State_Handler, {}},
	{write_flash_State, write_flash_State_Handler, {}},
	{switchedoff_State, switchedoffState_Handler2, {_stopcommand_Event, _resetcommand_Event, _tone_Event}},
    {pnp_State, pnpState_Handler2, {_keyup_Event, _fillingcomplete_Event, _tone_Event}},
	{filledmamo_State, filledmamo_State_Handler2, {_keypress_Event, _nozzledown_Event, _resetcommand_Event, _stopcommand_Event, _switchoffcommand_Event, _tone_Event}},
	{tone_entry_State, tone_entry_State_Handler2, {}},
	{last_State, 0, {}}
};
...
eSystemState nozzledown_Handler2(void)
{
	...
	//lcd_print_line2_2("nos dn");
	 reset_timer2(30);
	 stop_timer2();

//	 if(  ( (eLastState2 == idle_State) && (eNextState2 == idle_State) ) || ( (eLastState2 == filledmamo_State) && (eNextState2 == filledmamo_State) ) || (eNextState2 == nozzleup_waitingforauth_State) )
//	 {
////		  return idle_State;
//	 }
//	 else
//	 if( (eLastState2 == filling_State) && (eNextState2 == filling_State) )
//	 if(eNextState2 == filling_State)
	 if( (eLastState2 == filling_State) && (eNextState2 == filling_State) && (filling_mamo_flag2 == 0) && (hardwareError_flag2 == 0) )
	 {
		  do_calcs2();
		  get_time();
		  update_info();
...
//		  save_lastSale(operating_side);
		  ...
		  ////////////////////////////////////////////////
		  //---------------------------------------------
		  totalizer_saveStatus2 = SAVED_TO_MAIN_TOTALIZER;
...
/*
	...
	lcd_print_line3_2(str__);
 	 ...
 	 if( ((pump_LitreOverflow2 == 1) && (pulser_rem2 > 0 )) || (display_overflow2 == 1) || ( (authorizedSale_overflow2 == 1) && (pulser_rem2 > 0 ) ) )     //)    //|| ((display_overflow2 == 1) && (pulser_rem2 > 0 )) )
 	 {
...
//			mechTotalizer2 = (mechTotalizer2_ + amt_middle2);
					...
					mechTotalizer2 = (mechTotalizer2_ + pulser2amt2(current_pulser2));
			#else
					mechTotalizer2 = (mechTotalizer2_ + amt_middle2);
			#endif
...
eSystemState timeout_Handler2(void)
{
	 if(eLastState2 != authorised_nozzleup_State)
	 {
		 ...
		 if( ((pump_LitreOverflow2 == 1) && (pulser_rem2 > 0 )) || ((display_overflow2 == 1) && (pulser_rem2 > 0 )) || ( (authorizedSale_overflow2 == 1) && (pulser_rem2 > 0 ) )  )
		 {
			  if(pump_LitreOverflow2 == 1)
				  pump_LitreOverflow2 = 0;
			  if(display_overflow2 == 1)
	  ...
	  else if(eLastState2 == authorised_nozzleup_State)
	  {
		 ...
		 lcd_print_line1_2("    No ");
		 lcd_print_line2_2("  Flouu ");
		 lcd_print_line3_2("Err15 ");
	 }
          ...
          if( (kkey2 >= '0' )&&(kkey2 <= '9') )//if numerals
          {
        	if(_index2 > 7)
          ...
          else if (kkey2 == 'F' )  //if clear key,
			 {
               ...
               //lcd_print_line2_2("        ");  //clear second line.
               return 'F';
			 }
...
//				   }
				   else if( (index_menu == pass2) && (strcmp(pass_, password_level2) == 0) ) //level 2 access ?
				   {
					   ...
					   retrieve_otpSeed_session_fram(side_b);

					   if(otpSeed_session[1].otpSeed_flag == OTPSESSION_ON)
					   {
						   uint32_t time_stamp = RtcToInt(2019);
						   time_stamp = time_stamp - otpSeed_session[1].time_stamp;

						   if(time_stamp <= 4096)  //4096 translates to 1 hour
						   {
							   otp_seed2 = otpSeed_session[1].otp_seed;
						   }
						   else
						   {
							   otp_seed2 = generate_otpVariable2();
							   save_otpSeed_session_fram(side_b);
						   }
					   }
					   else
					   {
...
//				   {
//					  lcd_print_line2_2("---no---");
//					  HAL_Delay(1500);
//					  lcd_print_line2_2("        ");
//					  clear_buffer2();   	//clear the buffer
//					  lcd_print_line2_2(keypad_pw_xter2);
//					  lcd_print_line3_2("        ");
//				   }
				   else if(access == non)             //no match...
				   {
					  lcd_print_line2_2(" Access");
					  lcd_print_line3_2("Denied");
					  HAL_Delay(200);
					  lcd_print_line2_2("        ");
					  lcd_print_line3_2("        ");
					  HAL_Delay(200);
	    ...
	    if (t2 >= 500)  //time to display?
		{
			...
			lcd_print_line1_2(pass[index_menu]);  //show password level menu
			lcd_print_line2_2(keypad_pw_xter2);
			lcd_print_line3_2("        ");
			t2 = 0;
		}
   ...
   //=========================================================================
      else if ( (auth2 == authed) && (access == level1) && (fxn == nothing)  )
      {
   	   if (t2 >= 500)
   		 {
   			if(index_menu == 3) index_menu = 2;
   			lcd_print_line1_2(menu_level1[index_menu]);   //show menu item.
   			lcd_print_line2_2("        ");
   			t2 = 0;
   		 }
   ...
   //==================================================================================================
     ...
     else if ( (auth2 == pre_otp_authed) && ( (access == level2)||(access == level3) ) && (fxn == nothing)  )
     {
     		if (t2 >= 300)
			{
			  lcd_print_line1_2("OTP Seed");

			  snprintf(st__, sizeof(st__), "    %d", otp_seed2);
			  lcd_print_line2_2(st__);
...
//						lcd_print_line1_2(menu2[index_menu]);   //show menu item.
//						lcd_print_line2_2("        ");
//						t2 = 0;
//					 }
//                 // -------------- test keys....----------------
...
//							   save_otp(side_b);
					   ...
					   {
						   lcd_print_line1_2(" Access ");
						   lcd_print_line2_2(" Denied ");
						   lcd_print_line3_2("        ");
						   HAL_Delay(1000);
    	 	 ...
    	 	 if (t2 >= 500)
      		 {
      			if(access == level2)
      			{
      				...
      				lcd_print_line1_2(menu_level2[index_menu]);   //show menu item.
      			}
      			else if(access == level3)
   			{
      				if(index_menu == 23) index_menu = 0;
      				lcd_print_line1_2(menu_level3[index_menu]);   //show menu item.
   			}
...
//   			if(copy[pump_indx-1].mode == AUTO)
			...
			{
				lcd_print_line2_2(" Auto ");
			}
		    else
			{
				lcd_print_line2_2("NNanual ");
			}
...
//   		  if(copy[pump_indx-1].noz == overide)
   		  ...
   		  {
   			lcd_print_line2_2("Active");
   		  }
   		  else
   		  {
   			lcd_print_line2_2("Inactive");
   		  }
...
//   				 if (copy[pump_indx-1].noz == overide)
     ...
     //======================================================================
      else if ( (auth2 == authed) && ( (access == level2)||(access == level3) ) && (fxn == display_orientation)  )
      {
   	   ...
   	   if (t2 >= 300)
   	   {
   		   if(copy_stream1[1].display_format == PL)
   		   {
   			   lcd_print_line1_2("PL NNode");
   		   }
   		   else
   		   {
   			   lcd_print_line1_2("LP NNode");
   		   }
   		...
   		if ( (rd19 == 1)&&( key19_sto_ == 0) )
		 {
   		 ...
   		 if( (pkey > 0) )  //key detected
   		 {
   			 if (pkey == 'B')  // up key
			 {
				 ...
				 else if(copy_stream1[1].display_format == LP)
   	 ...
   	 if (t2 >= 300)
   	  {
   		 ...
   		 float pr = strtof(keyboard_entry2, &endPtr);
...
//   					{
   				copy_stream1[1].price_ = strtof(keyboard_entry2, &endPtr);
...
//   						}
   ...
   //==========================================================================
      if ( (auth2 == authed) &&  ( (access == level2)||(access == level3) ) && (fxn == set_no_flow_time)  )
      {
   	   if (t2 >= 500)
   		 {
   		   printDisp_i2(copy_stream2[pump_indx-1].timeOut_noFlow, 1, 0, 4, RT, CLEAR);
   		   lcd_print_line2_2(keyboard_entry2);
...
//   					 if (pump_indx == 2)   // if side b
//   						{
   				 copy_stream2[1].timeOut_noFlow =  atoi(keyboard_entry2);
...
//   						}
   ...
   //						  clear_logA();
   //						  lcd_print_line2_2("        ");
   //						  lcd_print_line2_2("cleared ");
   //						  HAL_Delay(2000);
   //						  clr_screen2(); //lcd_print_line2_2("        ");
   //						  fxn = nothing;
   //						}
   //
   //					 if (pump_indx == 2)   // if side b
    	   ...
    	   if (t2 >= 500)
    		 {
    		   if(chg_pw == 0)
    		   {
   			   ...
   			   lcd_print_line1_2(keypad_pw_xter2); //keyboard_entry2);
   			   lcd_print_line2_2("  Enter ");
   			   lcd_print_line3_2(ch_pass[index_generic]);  //show password level menu
    		   }
    		   else
    		   {
    			    lcd_print_line1_2(keypad_pw_xter2); //keyboard_entry2);
    			    lcd_print_line2_2("Re-enter");
   				lcd_print_line3_2(" Pass");
    		   }
    	    	t2 = 0;
            }
    	 // -------------- test keys....----------------
   ...
   //									 copy[1].passwd3 = pw1;
                                    ...
                                    HAL_Delay(1000);
                                    lcd_print_line1_2("Passuurd ");
                                    lcd_print_line2_2("Changed ");
                                    HAL_Delay(1000);
                                    fxn = nothing;
   								 chg_pw = 0;
   								 clear_buffer2();
      ...
      //		       snprintf(st__, sizeof(st__), "%d litre", calibrationCan_measure1);
      		       ...
      		       lcd_print_line1_2("  Start  ");

      //		       if (t >= 500)
      //			   {
      ////					if (vol_sold == l1)
      ////					{
      ////						lcd_print_line1_2("10 litre");
      ////					}
      ////					else
      //						lcd_print_line1_2(st__);
      //
      //				   if (pump_indx == 1)
      //					 {
      //						lcd_print_line3_2("  A  ");
      		  ...
      		  if(settings_stream1[1].mode == AUTO_MODE)
  			  {
					if(timer_go >= TIMEOUT_GO)   //if go's timeout is 5sec threshold
					{
						...
						if(calibration_flag2 == CALIBRATED)
						{
							  amt_middle2 = calibrationCan_measure2;
							  price_real2 = (calibrationCan_measure2 * litre_price2);
							  price_upper2 = (calibrationCan_measure1 * litre_price2);
							  amt_real2 = calibrationCan_measure2;
...
//      				  save_lastSale_fram(operating_side);
        ...
        //				  calib_pulser1 = __HAL_TIM_GET_COUNTER(&htim5);  //use hardware counter
      				  lcd_print_line1_2("Set your");
      				  lcd_print_line2_2("Volunne ");
      				  HAL_Delay(2000);
          ...
          if(volume_flag2 == 3)  //Enter the price.
      	   {
      			  if(t2 > 400)
      				  {
      					lcd_print_line1_2("Volunne ");

      					float vol = strtof(keyboard_entry2, &endPtr);
      					printDisp_f2(vol, 2, 0, 7, RT, CLEAR);
      					t2 = 0;
      				  }
      			  ...
      			  {
      				  float cal_vol = strtof(keyboard_entry2, &endPtr);
      				  lcd_print_line1_2("        ");   //clear screen.
      				  lcd_print_line2_2("        ");
      				  clear_buffer2();
      					//index_ = 0;
      	   ...
      	   if (t2 >= 500)
      		 {
      			...
      			if (ln_ == 3)
      			 {
      				lcd_print_line1_2(" Unit P ");
      			 }
             //---------------------------------
      		    if (dp_ == 1)
      			 {
      				lcd_print_line2_2(" 999999.9");
      			 }

      		    if (dp_ == 2)
      			 {
      				lcd_print_line2_2(" 99999.99");
      			 }

      		    if (dp_ == 3)
      			 {
      		    	lcd_print_line2_2(" 9999.999");
      			 }
      		    if (dp_ == 4)
      			 {
      				lcd_print_line2_2(" 999.9999");
      			 }
              //--------------------------------
//      			 if (pump_indx == 1)
//      			 {
...
//      					 }
      ...
      //		  char st__[10] = {0};
      		  ...
      		  {
      			  lcd_print_line1_2("1- Tinne");
      			  lcd_print_line2_2("2-  Date");
      			  t2 = 0;
...
//      		      if( (index_generic != 1) && (index_generic != 2) ) index_generic = 1;
      			  ...
      			  lcd_print_line1_2("Sides  ");

      			  snprintf(st__, sizeof(st__), "       %d", copy_stream2[1].noz_count);
      			  lcd_print_line2_2(st__);
      	 ...
      	 // -------------- test keys....----------------
      		...
      		if ( (rd19 == 1)&&( key19_sto_ == 0) )
			 {
      		 ...
      		 if( (pkey > 0) )  //key detected
      		 {
      			 if (pkey == 'B')  // up key
      			 {
      				 if(copy_stream2[1].noz_count == 1)
						 copy_stream2[1].noz_count = 2;
					 else if(copy_stream2[1].noz_count == 2)
        ...
        //======================================================================
         else if ( (auth2 == authed) && ( (access == level2)||(access == level3) ) && (fxn == suppressed_display_vol)  )
         {
			 ...
			 if(firstTime_sV == 1)
			 {
				   index_generic = (copy_stream2[1].startUp_suppressVol * 100);
				   firstTime_sV = 0;
			 }
...
//      		   if( (index_generic != 10) && (index_generic != 20) ) index_generic = 10;
      		   ...
      		   lcd_print_line1_2("Can Size");

      		   snprintf(st__, sizeof(st__), "    %02d L", copy_stream2[1].calibration_measureCan);
      		   lcd_print_line2_2(st__);
      	 ...
      	 // -------------- test keys....----------------
      		...
      		if ( (rd19 == 1)&&( key19_sto_ == 0) )
			 {
      		 ...
      		 if( (pkey > 0) )  //key detected
      		 {
      			 if (pkey == 'B')  // up key
      			 {
      				 if(copy_stream2[1].calibration_measureCan == 10)
    					 copy_stream2[1].calibration_measureCan = 20;
    				 else if(copy_stream2[1].calibration_measureCan == 20)
        ...
        //======================================================================
         else if ( (auth2 == authed) && ( (access == level2)||(access == level3) ) && (fxn == shift_login_type_)  )
         {
      	   ...
      	   if (t2 >= 300)
      	   {
      		   lcd_print_line1_2("Login.Typ");

      		   lcd_print_line2_2(login_type[copy_stream2[0].shift_login_type]);
      	 ...
      	 // -------------- test keys....----------------
      		...
      		if ( (rd19 == 1)&&( key19_sto_ == 0) )
      		 ...
      		 if( (pkey > 0) )  //key detected
      		 {
      			 if (pkey == 'B')  // up key
      			 {
     				 if(copy_stream2[1].shift_login_type == None_)
						 copy_stream2[1].shift_login_type = Code_;
					 else if(copy_stream2[1].shift_login_type == Code_)
						 copy_stream2[1].shift_login_type = Card_;
					 else if(copy_stream2[1].shift_login_type == Card_)
        ...
        //======================================================================
         else if ( (auth2 == authed) && ( (access == level2)||(access == level3) ) && (fxn == number_of_shifts_)  )
         {
      	   ...
      	   if (t2 >= 300)
      	   {
      		   lcd_print_line1_2("Shifts   ");

      		   snprintf(st__, sizeof(st__), "       %d", copy_stream2[1].number_of_shifts);
      		   lcd_print_line2_2(st__);
      	 ...
      	 // -------------- test keys....----------------
      		...
      		if ( (rd19 == 1)&&( key19_sto_ == 0) )
      		 ...
      		 if( (pkey > 0) )  //key detected
      		 {
      			 if (pkey == 'B')  // up key
      			 {
      				 if(copy_stream2[1].number_of_shifts == 1)
    					 copy_stream2[1].number_of_shifts = 2;
    				 else if(copy_stream2[1].number_of_shifts == 2)
        ...
        //======================================================================
         else if ( (auth2 == authed) && (access == level3) && (fxn == calibration_type_)  )
         {
      	   ...
      	   if (t2 >= 300)
      	   {
      		   	  if(copy_stream2[1].calibration_type ==  Wizard)
      			  {
      				lcd_print_line1_2("UUizard");
      			  }
      			  else
      			  {
      				lcd_print_line1_2("NNanual");
      			  }
      		...
      		if ( (rd19 == 1)&&( key19_sto_ == 0) )
			 {
      		 ...
      		 if( (pkey > 0) )  //key detected
      		 {
      			 if (pkey == 'B')  // up key
      			 {
    				 ...
    				 else if(copy_stream2[1].calibration_type == Manual_calib)
      		  ...
      		  if(index_generic == 0)
			  {
				 if(attendant2.session_id[0] != NULL)
				 {
					 ...
					 else
					 {
						 lcd_print_line1_2("l        ");
						 lcd_print_line2_2("        ");
						 printDisp_f2(shiftTotaliser_vol, 2, 0,5,RT,CLEAR ); //lcd_print_line2_2(scc);
					 }
				  }
				  else
				  {
       ...
       else if ( (auth2 == authed) && ( (access == level2)||(access == level3) ) && (fxn == keypress_tone_)  )
       {
         		...
         		if ( (rd19 == 1)&&( key19_sto_ == 0) )
				 {
         		 ...
         		 if( (pkey > 0) )  //key detected
         		 {
         			 if (pkey == 'B')  // up key
         			 {
         				 ...
         				 else if(copy_stream2[1].keypress_tone == No)
         				 {
      	...
      	//=====================================================================================================
      	else if ( (auth2 == authed) && (access == level3) && (fxn == communication_card)  )
      	   {
      		   ...
      		   if (t2 >= 300)
      		   {
      			   lcd_print_line1_2("Enforce ");

      			   if(copy_stream2[1].commCard_enforced ==  true)
      			   {
      				   lcd_print_line2_2("     Yes");
      			   }
      			   else
      			   {
      				   lcd_print_line2_2("      No");
      			...
      			if ( (rd19 == 1)&&( key19_sto_ == 0) )
				 {
      			 ...
      			 if( (pkey > 0) )  //key detected
      			 {
      				 if (pkey == 'B')  // up key
      				 {
    					 ...
    					 else if(copy_stream2[1].commCard_enforced == false)
    					 {
     	   ...
     	   if (pulser_flag2 == 0)
     	    {
     		       if (t2 >= 500)
     			   {
     					if (pulserr == 0)
     					{
     						lcd_print_line1_2("  Type  ");
     					}
     					else
     						lcd_print_line1_2(" Offset  ");
     		   ...
     		   if (t2 >= 500)
     		   {
     			   if (pulserr == 2)
     			   {
     				   if (copy_stream2[1].pulser_type_ == quadrature)
     					{
     						lcd_print_line1_2("Ouadrat. ");
     					}
     					else if (copy_stream2[1].pulser_type_ == non_quadrature)
     					{
     						lcd_print_line1_2("Non-Quad.");
     					}
     		         ...
     		         if(prog_revisit2 == 1)
     		    	 {
     		   	   ...
     		   	   if (valve_flag2 == 0)
     		   	   {
     				   if (t2 >= 500)
     				   {
     						if (_valve == 0)
     						{
     							lcd_print_line1_2("Ds Start");
     						}
     						else
     							lcd_print_line1_2("Disp. End");
     		   	...
     		   	else if(valve_flag2 == 1)
     			{
     				   if (t2 >= 300)
     				   {
     					   if (_valve == 2)
     					   {
     						    if(firstTime_v == 1)
    						    {
    						    	float_generic = copy_stream2[1].valve_salesStart;
    								firstTime_v =  0;
    						    }
     					   ...
     					   else if (_valve == 3)
     					   {
     						    if(firstTime_v == 1)
    							{
    								float_generic = copy_stream2[1].valve_salesEnd;
    								firstTime_v =  0;
    							}
...
//			}
	...
	if (settings_stream1[1].mode == AUTO_MODE)
	{
		if(online_calibFlag2 == 1)
		{
			settings_stream1[1].pi_cal = (calib_pulser2 / vol_effective2);
...
//					online_setUnitPrice2();
	...
	{
		 // lcd_print_line2_2("n up u a");
//			 clr_screen2();
		 lcd_print_line1_2("  Punnp   ");
		 lcd_print_line2_2(" Linnit  ");
		 lcd_print_line3_2("Err7   ");
		 if(t2 > 6000)
		 {
			 t2 = 0;
			 pump_LitreOverflow2 = 0;
	 ...
	 else if (display_overflow2 == 1)
	 {
		 if(firstTime_display_overflow2 == 1)
		 {
			 ...
			 {
				 clr_screen2();
				 lcd_print_line1_2("Display   ");
				 lcd_print_line2_2(" Linnit  ");
				 lcd_print_line3_2("Err18   ");
			 }
...
//					 lcd_print_line2_2(middle2);
	    ...
	    else if (authorizedSale_overflow2 == 1)
		{
			 if(firstTime_authorizedSale_overflow2 == 1)
			 {
				 if( (t2 > 2000) && (t2 <= 6000) )
				 {
					 clr_screen2();
	...
	//					 lcd_print_line2_2(middle2);
			 ...
			 else if(firstTime_authorizedSale_overflow2 == 0)
			 {
				 if( (t2 > 6000) && (t2 <= 12000) )
				 {
					 clr_screen2();
	  	...
	  	{
	  		 lcd_print_line1_2("No Unit   ");
			 lcd_print_line2_2("Price  ");
			 lcd_print_line3_2("Err8   ");
			 if(t2 > 6000)
			 {
				 t2 = 0;
				 _litre_price2 = 0;
	  	...
	  	{
	  		 lcd_print_line1_2("No-punnp   ");
			 lcd_print_line2_2(" Linnit  ");
			 lcd_print_line3_2("Err21   ");
			 if(t2 > 6000)
			 {
				 t2 = 0;
				 _pump_max_litres2 = 0;
...
//		{
//			 lcd_print_line1_2("  No   ");
//			 lcd_print_line2_2("Annount  ");
//			 lcd_print_line3_2("Err17   ");
//			 if(t2 > 6000)
//			 {
//				 t2 = 0;
//				 _auth_v2 = 0;
...
//		}
		...
		{
			 lcd_print_line1_2("   Non   ");
			 lcd_print_line2_2("  Valid  ");
			 lcd_print_line3_2("Err22   ");
			 if(t2 > 6000)
			 {
				 t2 = 0;
				 nonValid_sale2 = 0;
		...
		{
			 lcd_print_line1_2("  Back   ");
			 lcd_print_line2_2("  Flouu  ");
			 lcd_print_line3_2("Err13   ");
			 if(t2 > 6000)
			 {
				 t2 = 0;
				 idle_backwardPulse2 = 0;
		...
		{
			 lcd_print_line1_2("   Idle   ");
			 lcd_print_line2_2("  Flouu  ");
			 lcd_print_line3_2("Err14  ");
			 if(t2 > 6000)
			 {
				 t2 = 0;
				 idle_forwardPulse2 = 0;
		...
		{
			 lcd_print_line1_2("   Flouu   ");
			 lcd_print_line2_2("  Loss  ");
			 lcd_print_line3_2("Err16  ");
			 if(t2 > 6000)
			 {
				 t2 = 0;
				 flow_loss2 = 0;
...
eSystemState authorised_nozzledown_State_Handler2(void)
{
   ...
   if(t2 > 300)
   {
	  lcd_print_line1_2(" Pic    ");
	  lcd_print_line2_2("No22le  ");
	  char str__[8]= {0};
	  snprintf(str__, sizeof(str__), "%.2f", litre_price2); lcd_print_line3_2(str__);
      t2 = 0;
   }
...
//			  half_litre2 = (0.5 * litre_price2);
				...
				if (sellmode2 == P)
				{
					  ...
					  if( (pump_LitreOverflow2 == 1) || (display_overflow2 == 1) )
					  {
						 sales_type2 = UNPROGRAMMED_SALE;
					  }
					  else
					  {
						 sales_type2 = PRICE_PROGRAMMED;
					  }
...
//						key_value2 = auth_p2;
					...
					if( (key_value_sellmodeL2 == 1) && (authorizedSale_overflow2 == 0) && (pump_LitreOverflow2 == 0) && (display_overflow2 == 0) )
					{
					...
					if( (authorizedSale_overflow2 == 1) || (pump_LitreOverflow2 == 1) || (display_overflow2 == 1) )
					{
						sales_type2 = UNPROGRAMMED_SALE;
					}
			  ...
			  else
			  {
				  ...
				  {
					  if(auth_p2 > sellPrice_max_pump)
					  {
						  auth_p2 = sellPrice_max_pump;
						  pump_LitreOverflow2 = 1;
					  }

					  else if(auth_p2 > sellPrice_max_dpp)
					  {
...
//					key_value_ = (sellPrice_max_dpp / litre_price2);
					...
					///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
					if( (key_value_sellmodeP2 == 1) && (authorizedSale_overflow2 == 0) && (pump_LitreOverflow2 == 0) && (display_overflow2 == 0) )
					{
						key_value2 = key_value_original2;
					}
					///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				{
					if( (auth_v2 > pump_max_litres2) || (auth_v2 > key_value_) )
					{
						  if(auth_v2 > pump_max_litres2)
						  {
							  auth_v2 = pump_max_litres2;
							  pump_LitreOverflow2 = 1;
						  }

						  else if (auth_v2 > key_value_)
						  {
							  auth_v2 = key_value_;
							  display_overflow2 = 1;
						  }
...
eSystemState savesettings_State_Handler2(void)
{
	...
	if (sav == 0)
		{
		  if(t2 > 500)
		   {
			  lcd_print_line1_2("1-  Saue");
			  lcd_print_line2_2("2-Cancel");
			  t2 = 0;
		   }
		  respons = atoi(keyboard_entry2);
        //------------------------------------------------
   ...
   //=======================================
	if (sav == 1)
		{
		 	lcd_print_line1_2(" Sauing ");
		 	lcd_print_line2_2("Settings");

		 	copy_settings(move_to_settings0);
//		 	save_settings_original();
		 	save_settings_original_fram(side_b);
...
// int cnv = 0;
// snprintf(str_, sizeof(str_), "%.2f", price); lcd_print_line2_2(str_);
// snprintf(str_, sizeof(str_), "%.2f", amt);   lcd_print_line2_2(str_);

 lcd_print_line1_2("--auth2--");
 lcd_print_line2_2("        ");
 char str__[8]= {0};
 snprintf(str__, sizeof(str__), "%.2f", litre_price2); lcd_print_line2_2(str__);
...
//			price_upper2 += 0.00011;
			...
			price_upper2 = strtof(temp, &endPtr);
...
//		}
	...
	//	if(calib_pulser1 < 15800)  //15987, 15967 .... 1106247681
	 ...
	 if  (t2 > 500)
	 {
		 lcd_print_line1_2("  Auto  ");
		 lcd_print_line2_2(" NNode  ");
...
void keypad_fillingUpdate2(void)
{
	...
	if(sellmode2 == P)
	{
	  ...
	  //---------------------------------
		...
		if( (strchr(upper2, '.')) && (indexx_ > 6) ) space = 8;
		while(space > 0)
		{
		   keyboard2[ind++] = upper2[indexx_ - space];
		   space--;
		}
	...
	else if(sellmode2 == L)
	{
	  ...
	  //---------------------------------
		...
		if( (strchr(middle2, '.')) && (indexx_ > 6) ) space = 8;
		while(space > 0)
		{
		   keyboard2[ind++] = middle2[indexx_ - space];
		   space--;
		}
