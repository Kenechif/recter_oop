...
////using namespace std;
//
////typedef void (*uart_function_cb)(unsigned char*, unsigned int);
//
...
//			void process_data(); 						//GETs each data from the raw data
//
//
...
//	int auth_by_vol(float vol);	// authorize the pump's volume
//	int auth_by_amo(float amnt);	// authorize the pump's amount
//	int set_amo(float amnt);	//this method is peculiar to dart protocol only and this is used in filling state to ensure that the filled amount does not exceed its limit
//	void authorise();	// authorise the pump with max vol of 10000
//	void stop();
//	void pause();
//	void resume();
//	void switch_off();				// switch off the pump
//	void reset();					// reset the pump
//
